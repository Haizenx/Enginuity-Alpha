  ...
  const selectedItemsForEstimate = useMemo(() => {
    ...
    return items
      ...
      .map((it, idx) => {
        ...
        const unitPrice = getFinalUnitPriceForItem(it) ?? 0;
  ...
  const executePdfGeneration = () => {
    ...
    sortedItemsForPDF.forEach(item => {
        ...
        if (itemCategory !== currentCategoryInPDF) {
            tableRows.push([{
                content: itemCategory.toUpperCase(), colSpan: 6,
        ...
        const unitPriceForTable = getFinalUnitPriceForItem(item);
        const amount = (unitPriceForTable ?? 0) * quantity;
        tableRows.push([
            item.itemNo || itemCounterForPDF++,
            item.description || item.name || "N/A",
            quantity,
            item.unit || 'pc',
            formatCurrencyForPDF(unitPriceForTable),
    ...
    if (tableRows.length > 0) {
        autoTable(doc, {
            startY: startY,
            head: [['ITEM NO.', 'ITEM DESCRIPTION', 'QTY', 'UNIT', 'UNIT PRICE', 'AMOUNT']],
            ...
            columnStyles: {
                0: { cellWidth: 18, halign: 'center' }, 1: { cellWidth: 125 }, 2: { cellWidth: 15, halign: 'right' },
                3: { cellWidth: 18, halign: 'center' }, 4: { cellWidth: 35, halign: 'right' }, 5: { cellWidth: 35, halign: 'right' },
            },
              ...
              <tbody className="text-sm">
                {selectedItemsForEstimate.map(row => (
                  <tr key={`${row.no}-${row.name}`} className="hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3 border text-right">{row.unitPrice.toFixed(2)}</td>
                    <td className="py-2 px-3 border text-right">{row.amount.toFixed(2)}</td>
                  </tr>
                ))}
                <tr>
                  <td className="py-2 px-3 border font-semibold" colSpan={5}>TOTAL</td>
